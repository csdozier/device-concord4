...
class AlarmPanelInterface(object):
    ...
    def send_message(self, msg, retry=False):
        ...
        """
        self.logger.debug('SERIAL TX -> 0x'+str(binascii.hexlify(bytearray(msg))))
        self.tx_pending = msg
        if retry:
    ...
    def handle_message(self, msg):
        ...
        self.logger.debug('SERIAL RX <- 0x'+str(binascii.hexlify(bytearray(msg))))
    ...
    def send_nak(self):
        self.logger.debug('SERIAL TX -> NAK')
        self.serial_interface.write(NAK)
    def send_ack(self):
        self.logger.debug('SERIAL TX -> ACK')
